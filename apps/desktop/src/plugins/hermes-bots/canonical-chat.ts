 ...
 * touching the primary renderer. Local rows may create their registry row;
 * thin remote rows fail closed when the row is absent so a lookup miss never
 * mutates a remote source. */
export async function openBotCanonicalChatInWindow(owner: RosterRow | string): Promise<string> {
  ...
  const storedId =
    ...
    existing?.id ||
    (await createCanonicalChat(owner, {
      // A pop-out must not claim or navigate the primary renderer.
  ...
  await host.openSessionWindow(String(storedId), {
    ...(route?.connectionId
      ...
      : bot?.connectionId
        ...
        : {}),
    // The native window boots through the Desktop-side route profile. RPCs
    // may separately rewrite to targetProfile for aliases.
