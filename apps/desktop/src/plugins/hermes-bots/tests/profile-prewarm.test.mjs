...
test('behavior: context-menu pin mutation hydrates a non-identity alias before toggling', async () => {
  const bot = {
    ...
    name: 'worker',
    // A rich source-scoped row has owner metadata available, so its mutation
    // menu may hydrate and save against that exact route. Thin remote rows set
    // remoteSource and intentionally expose only the non-mutating pop-out.
    sourceScoped: true,
    route: {
      connectionId: 'remote-a',
      mode: 'remote',
