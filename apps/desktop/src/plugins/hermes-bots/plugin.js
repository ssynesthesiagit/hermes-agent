...
// pin, avatar, group, unread state, or canonical-chat id. The ONE
...
/** Open the same name-resolved Bot Chat in a focused native window without
 *  touching the primary renderer. The canonical resolver is deliberately run
 *  with a false navigation fence. Local rows may create their registry row;
 *  thin remote rows are read-only and must fail closed when the row is absent,
 *  since a miss must never mutate the remote source. */
async function openBotCanonicalChatInWindow(owner) {
  if (typeof host.openSessionWindow !== 'function') {
    throw new Error('This Hermes Desktop version cannot open chat windows')
  }

  const { bot, name, route } = botOwner(owner)
  const existing = await findExistingCanonicalChat(owner)

  if (bot?.remoteSource && !existing) {
    throw new Error(`Could not resolve ${name}'s Bot Chat — remote Bot Chats are read-only and cannot be created here`)
  }

  const storedId = existing?.resolved_id || existing?.id || await createCanonicalChat(owner, {
    openingStillCurrent: () => false
  })
  ...
  await host.openSessionWindow(String(storedId), {
    ...(route?.connectionId
      ? { connectionId: route.connectionId }
      : bot?.connectionId
        ? { connectionId: bot.connectionId }
        : {}),
    // The window boot route identifies the Desktop-side profile (`route.profile`),
    // while requestForBot deliberately rewrites RPC params to `targetProfile`.
    // Passing the backend target here can dial the wrong profile when the two
    // identities differ (for example an alias on a remote gateway).
    profile: route?.profile || bot?.name || name
  })
...
function BotRow({ bot, onDelete, onEdit, onGroup, showHandle }) {
  ...
  // resolved canonical chat, preview THAT session — not the
  ...
  const openInWindow = async () => {
    ...
    try {
      await openBotCanonicalChatInWindow(bot)
    } catch (error) {
...
function CreateAgentDialog({ open, onClose, roster }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // the first thing the user sees; the title registry is durable from
      // minute one and no session-id pointer is persisted.
      try {
        // Creates, opens, and kicks off the intro in one flow. This is
