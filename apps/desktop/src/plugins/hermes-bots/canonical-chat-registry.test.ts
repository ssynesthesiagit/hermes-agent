...
const { hostMock, persistMock, requestForBotMock, saveBotMetaMock } = vi.hoisted(() => ({
  hostMock: { openSession: vi.fn(), openSessionWindow: vi.fn(), request: vi.fn() },
  persistMock: vi.fn(),
...
beforeEach(() => {
  ...
  hostMock.openSessionWindow.mockResolvedValue(undefined)
})
...
describe('native Bot Chat windows preserve source identity', () => {
  it('opens an existing local canonical chat without navigating the main renderer', async () => {
    respondWith(method =>
      method === 'session.list' ? { sessions: [{ id: 'root-1', resolved_id: 'tip-2', title: 'Bot Chat' }] } : {}
    )

    const { openBotCanonicalChatInWindow } = await loadModule()

    await expect(openBotCanonicalChatInWindow('ops')).resolves.toBe('tip-2')
    expect(hostMock.openSessionWindow).toHaveBeenCalledWith('tip-2', { profile: 'ops' })
    expect(hostMock.openSession).not.toHaveBeenCalled()
  })

  it('uses the Desktop alias profile while retaining the owning connection', async () => {
    respondWith(method =>
      method === 'session.list' ? { sessions: [{ id: 'remote-chat', title: 'Bot Chat' }] } : {}
    )
    ...
    await openBotCanonicalChatInWindow(owner)
    expect(hostMock.openSessionWindow).toHaveBeenCalledWith('remote-chat', {
      connectionId: 'tailnet-a',
      profile: 'worker'
    })
  })

  it('does not create a remote canonical chat when the registry lookup misses', async () => {
    const calls = respondWith(() => ({ sessions: [] }))
    const owner = { connectionId: 'tailnet-a', name: 'worker', remoteSource: true } as RosterRow
    const { openBotCanonicalChatInWindow } = await loadModule()

    await expect(openBotCanonicalChatInWindow(owner)).rejects.toThrow(/read-only/)
    expect(calls.some(call => call.method === 'session.create')).toBe(false)
    expect(hostMock.openSessionWindow).not.toHaveBeenCalled()
  })
})
