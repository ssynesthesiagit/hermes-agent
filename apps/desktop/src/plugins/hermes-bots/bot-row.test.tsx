...
const {
  ...
  vi.hoisted(() => ({
    ...
    notifyError: vi.fn(),
    openBotCanonicalChatInWindow: vi.fn(),
    openProfileSessions: vi.fn(),
    openRosterBot: vi.fn(),
...
vi.mock('@hermes/plugin-sdk', async importOriginal => {
  ...
  return {
    ...sdk,
    host: { ...sdk.host, ensureAgent, notifyError, openProfileSessions, requestProfile, warmAgent, warmProfile },
...
beforeEach(() => {
  ...
  openBotCanonicalChatInWindow.mockResolvedValue('bot-chat')
  openProfileSessions.mockResolvedValue(undefined)
...
describe('the menu carries the explicit ask for the forever-chat', () => {
  ...
  it('opens the canonical Bot Chat in a native window without activating another agent', async () => {
    const bot = {
      connectionId: 'work',
      name: 'research',
      remoteSource: true,
    ...
    fireEvent.contextMenu(renderRow(bot))
    fireEvent.click(await screen.findByText('Open Bot Chat in New Window'))

    await vi.waitFor(() => expect(openBotCanonicalChatInWindow).toHaveBeenCalledWith(bot))
    expect(ensureAgent).not.toHaveBeenCalled()
  })

  it('opens the owning profile Sessions pane and reports failures', async () => {
    const bot = { connectionId: 'work', name: 'research' } as RosterRow

    fireEvent.contextMenu(renderRow(bot))
    fireEvent.click(await screen.findByText('Sessions'))

    await vi.waitFor(() => expect(openProfileSessions).toHaveBeenCalledWith('work', 'research'))

    openProfileSessions.mockRejectedValueOnce(new Error('sessions unavailable'))
    fireEvent.contextMenu(screen.getByRole('button'))
    fireEvent.click(await screen.findByText('Sessions'))
    await vi.waitFor(() => expect(notifyError).toHaveBeenCalledWith(expect.any(Error), expect.stringMatching(/sessions/i)))
  })
})
