# Fresh Codex task handoff

Use this prompt for a new coding task:
...
> model selection, local-profile A2A routing, Desktop Bots Sessions action, and
> compact stable bot-chat popouts. Keep Hermes authentication, backend behavior,
> profile isolation, audit records, and bounded A2A safeguards intact. Never
> commit credentials, sessions, memories, model weights, generated packages,
> recovery archives, or machine-specific private state. Do not update a live
> installation directly: checkpoint, integrate on a temporary branch, run
