...
class PluginSettings:
    ...
    def from_context(cls, ctx: Any) -> "PluginSettings | None":
        ...
        try:
            ...
            capsule_path = ctx.get_config("capsule_path")
            receipt_path = ctx.get_config("receipt_path")
            now = ctx.get_config("now")
            scope = ctx.get_config("scope")
            ...
            for key in _REQUIRED_SCOPE_FIELDS:
                ...
                if key in _BOOLEAN_SCOPE_FIELDS:
                    ...
                    return None
            runtime = ctx.get_config("runtime_identity")
            ...
            return cls(
                ...
                cache_path=ctx.get_config("cache_path"),
                now=now,
                scope=dict(scope),
                runtime_identity=dict(runtime) if runtime is not None else None,
                compiler_version=ctx.get_config("compiler_version"),
                schema_generation=str(ctx.get_config("schema_generation", "yatima.k3.v1")),
                executable_hash=ctx.get_config("executable_hash"),
            )
        except Exception:
...
class K3HookRuntime:
    ...
    def on_pre_llm_call(self, **kwargs: Any) -> dict[str, str] | None:
        ...
        if any(key not in kwargs or kwargs[key] != scope.get(key) for key in _REQUIRED_SCOPE_FIELDS):
            return None
        try:
